...
/* KOLDIS 0.9.7 — Supabase accounts integration
...
const defaults={page:'home',budget:60,store:'',goals:['High Protein'],likes:[],dislikes:[],intolerances:[],method:'Egal',plan:{},plans:[{id:'w1',label:'Woche 1',plan:{}}],currentPlan:0,cart:[],customShopping:[],shoppingChecked:{},saved:[],filter:'Alle',filters:[],query:'',theme:'dark',portionDefault:4,onboardingDone:false};
function load(){
  try{
    ...
    if(!Array.isArray(st.filters)) st.filters=(st.filter && st.filter!=='Alle')?[st.filter]:[];
    st.filters=st.filters.filter(f=>FILTERS.includes(f) && f!=='Alle');
    st.filter=st.filters[0]||'Alle';
...
function matchesFilter(r,f){switch(f){case'High Protein':return r.p>=40;case'Günstig':return r.price<=3.5;case'Schnell':return r.tags.includes('Schnell')||r.method==='Pfanne'||r.method==='Mikrowelle'||r.method==='Airfryer';case'Meal Prep':return r.tags.includes('Meal Prep');case'Low Calorie':return r.kcal<=550;case'Gesünder essen':return r.tags.includes('Gesünder essen');default:return true}}
function activeFilters(){return Array.isArray(state.filters)?state.filters.filter(f=>FILTERS.includes(f)&&f!=='Alle'):[];}
function matches(r){if(excluded(r))return false;const q=state.query.trim().toLowerCase();if(q&&!(`${r.name} ${r.ingredients.join(' ')} ${r.tags.join(' ')}`).toLowerCase().includes(q))return false;return activeFilters().every(f=>matchesFilter(r,f));}
...
function recipesPage(){const list=sortedRecipes();const selected=activeFilters();shell(`<section class="recipes"><div class="eyebrow">REZEPTE</div><h1>Finde dein Essen</h1><p class="lead">Aus passenden Gerichten findest du schnell das, was zu deinen Vorlieben, Zielen und deinem Budget passt.</p><div class="search"><span>🔎</span><input id="q" value="${esc(state.query)}" placeholder="Hähnchen, Pasta, Kartoffeln …"></div><div class="filter-note">Mehrere Filter gleichzeitig möglich · ${selected.length?selected.length+' aktiv':'keiner aktiv'}</div><div class="filters">${FILTERS.map(f=>`<button class="filter ${f==='Alle'?(selected.length===0?'active':''):(selected.includes(f)?'active':'')}" data-filter="${f}">${f}</button>`).join('')}</div><div class="result"><strong>${list.length} Gerichte</strong><span>aus ${RECIPES.length} Rezepten</span><button id="reset" class="link">Zurücksetzen</button></div><div class="cards">${list.length?list.map(recipeCard).join(''):'<div class="empty">Keine passenden Rezepte gefunden.<br><button id="reset2" class="secondary">Filter zurücksetzen</button></div>'}</div></section>`);const q=app.querySelector('#q');q.oninput=()=>{state.query=q.value;renderRecipesOnly()};app.querySelectorAll('[data-filter]').forEach(b=>b.onclick=()=>{const f=b.dataset.filter;if(f==='Alle'){state.filters=[];}else{const current=new Set(activeFilters());current.has(f)?current.delete(f):current.add(f);state.filters=[...current];}state.filter=state.filters[0]||'Alle';save();recipesPage()});app.querySelector('#reset').onclick=()=>{state.filters=[];state.filter='Alle';state.query='';save();recipesPage()};const r2=app.querySelector('#reset2');if(r2)r2.onclick=()=>{state.filters=[];state.filter='Alle';state.query='';save();recipesPage()};bindRecipeActions();}
function renderRecipesOnly(){const list=sortedRecipes();const cards=app.querySelector('.cards');const result=app.querySelector('.result');const selected=activeFilters();const note=app.querySelector('.filter-note');if(note)note.textContent=`Mehrere Filter gleichzeitig möglich · ${selected.length?selected.length+' aktiv':'keiner aktiv'}`;app.querySelectorAll('[data-filter]').forEach(b=>{const f=b.dataset.filter;b.classList.toggle('active',f==='Alle'?(selected.length===0):selected.includes(f));});result.innerHTML=`<strong>${list.length} Gerichte</strong><span>aus ${RECIPES.length} Rezepten</span><button id="reset" class="link">Zurücksetzen</button>`;cards.innerHTML=list.length?list.map(recipeCard).join(''):'<div class="empty">Keine passenden Rezepte gefunden.<br><button id="reset2" class="secondary">Filter zurücksetzen</button></div>';app.querySelector('#reset').onclick=()=>{state.filters=[];state.filter='Alle';state.query='';save();recipesPage()};const r2=app.querySelector('#reset2');if(r2)r2.onclick=()=>{state.filters=[];state.filter='Alle';state.query='';save();recipesPage()};bindRecipeActions()}
